...
def run_unit_tests(coverage: bool = True, fail_under: int = 50) -> int:
    ...
    cmd = ["uv", "run", "pytest"] + test_modules + migration_modules + [
        "-v",
        "--tb=short", 
...
def run_integration_tests() -> int:
    ...
    cmd = [
        ...
        "-v",
        "--tb=short",
        "-k", "not (ssh and network)",  # Skip tests requiring real SSH/network
...
def run_quality_checks() -> int:
    ...
    checks = [
        ...
        (["uv", "run", "mypy", "src", "--ignore-missing-imports"], "MyPy Type Checking"),
    ]
...
def main():
    ...
    parser.add_argument(
        "mode",
        choices=["unit", "integration", "e2e", "failing", "coverage", "quality", "ci", "all"],
        help="""Test mode:
