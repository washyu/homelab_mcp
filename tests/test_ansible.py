    ...
    async def test_ansible_variable_substitution(self):
        ...
        with patch('src.homelab_mcp.service_installer.AnsibleRunner') as mock_runner_class:
            ...
            with patch.object(self.installer, 'check_service_requirements') as mock_check:
                ...
                call_args = mock_runner.run_playbook.call_args if hasattr(mock_runner.run_playbook, 'call_args') else None
...
class TestAnsiblePlaybookRunner:
    ...
    async def test_ansible_runner_basic_execution(self):
        ...
        with patch('src.homelab_mcp.service_installer.subprocess.run') as mock_run:
                ...
                "plays": [
                    {
                        "play": {"name": "Test Play"},
                        "tasks": [
                            {"task": {"name": "Test Task"}, "host": "localhost", "status": "ok"}
                        ]
                    }
                ]
            })
            ...
            inventory = {"all": {"hosts": {"test-host": {"ansible_host": "192.168.1.100"}}}}
...
class TestAnsibleServiceTemplateProcessing:
            ...
            "installation": {
                ...
                "ansible": {
                    ...
                    "pre_tasks": [
                        ...
                        {"name": "Install Docker", "shell": "curl -fsSL https://get.docker.com | sh"}
                    ],
                    "tasks": [
                        {
    ...
    def test_ansible_template_conditional_tasks(self):
        ...
        with patch('src.homelab_mcp.service_installer.TEMPLATES_DIR', self.template_dir):
            ...
            ssl_task = next((t for t in tasks if "ssl_enabled | bool" in str(t.get("when", ""))), None)
            assert ssl_task is not None
            
            http_task = next((t for t in tasks if "not ssl_enabled | bool" in str(t.get("when", ""))), None)
