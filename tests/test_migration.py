...
class TestDatabaseMigrator:
    ...
    def test_migrate_device_history_success(self):
        ...
        changes = [
            MockDevice.create_sample_history(source_device_id)
        ]
...
class TestMigrationFunctions:
    ...
    def test_migrate_sqlite_to_postgresql_missing_psycopg2(self):
        ...
        with patch('builtins.print'):  # Suppress output
            with patch('src.homelab_mcp.migration.PostgreSQLAdapter', side_effect=ImportError("No module named 'psycopg2'")):
...
class TestMigrationCLI:
    ...
    @patch('src.homelab_mcp.migration.setup_postgresql_database')
    @patch('sys.argv', ['migration.py', 'setup', '--password', 'testpass', '--host', 'custom-host', '--port', '5433'])
    ...
    @patch('src.homelab_mcp.migration.migrate_sqlite_to_postgresql')
    @patch('sys.argv', ['migration.py', 'migrate', '--password', 'testpass', '--dry-run', '--sqlite-path', '/custom/path.db'])
