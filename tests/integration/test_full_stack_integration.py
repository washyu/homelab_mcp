...
from src.homelab_mcp.sitemap import NetworkSiteMap, discover_and_store, bulk_discover_and_store
...
from src.homelab_mcp.ssh_tools import setup_remote_mcp_admin, verify_mcp_admin_access, ssh_discover_system
...
def mock_network():
    ...
    network.add_mock_device("web-server-01", "192.168.1.10", {
        "cpu": {"model": "Intel Core i5", "cores": "4"},
        "memory": {"total": "8G", "used": "4G", "free": "3G", "available": "6G"}
    })
    
    network.add_mock_device("db-server-01", "192.168.1.20", {
        "cpu": {"model": "AMD EPYC", "cores": "16"},
        "memory": {"total": "64G", "used": "32G", "free": "20G", "available": "40G"}
    })
    
    network.add_mock_device("nas-server-01", "192.168.1.30", {
        "cpu": {"model": "Intel Atom", "cores": "4"},
        "memory": {"total": "4G", "used": "2G", "free": "1G", "available": "3G"},
        "disk": {"size": "4T", "used": "2T", "available": "1.8T", "use_percent": "50%"}
    })
    ...
    async def test_bulk_discovery_workflow(self, full_stack_env, mock_network):
        ...
        with patch('src.homelab_mcp.ssh_tools.ssh_discover_system', side_effect=mock_ssh_discover):
            ...
            targets = [
                ...
                {"hostname": "db-server-01", "username": "admin", "password": "pass"},
                {"hostname": "nas-server-01", "username": "admin", "password": "pass"},
                {"hostname": "unreachable-host", "username": "admin", "password": "pass"}  # Will fail
            ]
            ...
            successful = sum(1 for r in result["results"] 
                           if r.get("discovery_status") == "success" or r.get("status") == "success")
    ...
    async def test_discovery_to_service_deployment(self, full_stack_env, mock_network):
        ...
        with patch('src.homelab_mcp.ssh_tools.ssh_discover_system', side_effect=mock_ssh_discover):
            with patch.object(ServiceInstaller, 'install_service', side_effect=mock_install_service):
                ...
                web_server_target = next((t for t in monitoring_targets if t["hostname"] == "web-server-01"), None)
                ...
                with patch.object(installer, 'get_available_services', return_value=['nginx']):
                    with patch.object(installer, 'check_service_requirements', return_value={"requirements_met": True}):
                ...
                mock_network.add_mock_service("web-server-01", "nginx", {
                    "status": "running",
                    "ports": ["8080:80"],
    ...
    async def test_multi_device_service_orchestration(self, full_stack_env, mock_network):
        ...
        with patch('src.homelab_mcp.ssh_tools.ssh_discover_system', side_effect=mock_ssh_discover):
            with patch.object(ServiceInstaller, 'install_service', side_effect=mock_install_service):
                ...
                discovery_targets = [
                    "web-server-01", "db-server-01", "nas-server-01"
                ]
                ...
                with patch.object(installer, 'get_available_services', return_value=['nginx', 'postgres', 'redis']):
                    with patch.object(installer, 'check_service_requirements', return_value={"requirements_met": True}):
    ...
    async def test_service_to_vm_deployment_workflow(self, full_stack_env, mock_network):
        ...
        with patch('src.homelab_mcp.ssh_tools.ssh_discover_system', side_effect=mock_ssh_discover):
            with patch('src.homelab_mcp.vm_operations.deploy_vm', side_effect=mock_deploy_vm):
                with patch('src.homelab_mcp.vm_operations.get_vm_status', side_effect=mock_get_vm_status):
                    ...
                    with patch('src.homelab_mcp.vm_operations.list_vms_on_device', side_effect=mock_list_vms):
    ...
    async def test_complete_homelab_setup_workflow(self, full_stack_env, mock_network):
        ...
        tool_responses = {
            "ssh_discover": lambda args: {
                "content": [{"type": "text", "text": mock_network.get_device_discovery_result(args["hostname"])}]
            },
            ...
            "get_network_sitemap": lambda args: {
                "content": [{"type": "text", "text": json.dumps({
                    ...
                    "total_devices": 3,
                    "devices": [
                        {"hostname": "web-server-01", "status": "success", "connection_ip": "192.168.1.10"},
                        {"hostname": "db-server-01", "status": "success", "connection_ip": "192.168.1.20"},
                        {"hostname": "nas-server-01", "status": "success", "connection_ip": "192.168.1.30"}
                    ]
                })}]
            },
            ...
            "suggest_deployments": lambda args: {
                "content": [{"type": "text", "text": json.dumps({
                    ...
                    "suggestions": {
                        "monitoring_targets": [
                            {"hostname": "web-server-01", "connection_ip": "192.168.1.10"},
                            {"hostname": "db-server-01", "connection_ip": "192.168.1.20"},
                            {"hostname": "nas-server-01", "connection_ip": "192.168.1.30"}
                        ],
                        "database_candidates": [
                            {"hostname": "db-server-01", "cpu_cores": 16, "memory_total": "64G"}
                        ]
                    }
                })}]
            },
        ...
        with patch('src.homelab_mcp.tools.execute_tool', side_effect=mock_execute_tool):
            ...
            vm_result = await execute_tool("deploy_vm", {
    ...
    async def test_monitoring_deployment_workflow(self, full_stack_env, mock_network):
        ...
        async def mock_execute_tool(tool_name, args):
            if tool_name == "discover_and_map":
                ...
                return {
                    "content": [{"type": "text", "text": mock_deploy_monitoring_vm(
                        args["device_id"], args["platform"], args["vm_name"], args["vm_config"]
                    )}]
                }
            else:
        ...
        with patch('src.homelab_mcp.tools.execute_tool', side_effect=mock_execute_tool):
            ...
            discovery_result = await execute_tool("discover_and_map", {
                "hostname": "web-server-01",
                "username": "admin", 
                "password": "pass"
            })
            ...
            for tool in monitoring_tools:
                vm_result = await execute_tool("deploy_vm", {
                    ...
                    "vm_config": {
                        ...
                        "ports": ["9090:9090"] if tool == "prometheus" else ["3000:3000"] if tool == "grafana" else ["9100:9100"] if tool == "node-exporter" else ["9093:9093"]
                    }
                })
...
class TestErrorRecoveryAndRollback:
    ...
    async def test_partial_deployment_failure_recovery(self, full_stack_env, mock_network):
        ...
        async def mock_execute_tool(tool_name, args):
            if tool_name == "discover_and_map":
                ...
                return {
                    "content": [{"type": "text", "text": mock_deploy_vm_with_failures(
                        args["device_id"], args["platform"], args["vm_name"], args["vm_config"]
                    )}]
                }
            else:
        ...
        with patch('src.homelab_mcp.tools.execute_tool', side_effect=mock_execute_tool):
            ...
            # Discover device
            await execute_tool("discover_and_map", {
                "hostname": "web-server-01",
                "username": "admin",
                "password": "pass"
            })
    ...
    async def test_discovery_failure_handling(self, full_stack_env, mock_network):
        ...
        with patch('src.homelab_mcp.ssh_tools.ssh_discover_system', side_effect=mock_ssh_discover_with_failures):
            
            # Attempt bulk discovery with some failures expected
            targets = [
                {"hostname": "web-server-01", "username": "admin", "password": "pass"},
                {"hostname": "unreachable-host", "username": "admin", "password": "pass"}, 
            ...
            successes = sum(1 for r in result["results"] 
                          if r.get("discovery_status") == "success" or r.get("status") == "success")
            failures = sum(1 for r in result["results"]
                         if r.get("discovery_status") == "error" or r.get("status") == "error")
...
async def test_complete_homelab_lifecycle(full_stack_env, mock_network):
    ...
    with patch('src.homelab_mcp.ssh_tools.ssh_discover_system', side_effect=mock_ssh_discover):
        ...
        with patch('src.homelab_mcp.vm_operations.deploy_vm') as mock_deploy:
            ...
            monitoring_result = await deploy_vm(1, "docker", "prometheus", {
                "image": "prom/prometheus:latest",
                "ports": ["9090:9090"]
            })
