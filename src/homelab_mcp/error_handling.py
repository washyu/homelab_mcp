...
def timeout_wrapper(timeout_seconds: float = 30.0, default_response: Optional[Dict] = None):
    """
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt < max_retries:
                        logger.warning(f"Attempt {attempt + 1}/{max_retries + 1} failed for '{func.__name__}': {str(e)}. Retrying in {delay}s...")
...
async def safe_json_response(data: Union[str, Dict], fallback_message: str = "Operation completed") -> Dict[str, Any]:
    """
...
class HealthChecker:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            "status": "healthy" if (self.request_count == 0) or (self.error_count < self.request_count * 0.5) else "degraded",
